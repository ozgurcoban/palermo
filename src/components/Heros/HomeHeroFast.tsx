...
// Use public URL path for better caching
const heroImageUrl = "/hero.webp";

// Inline critical styles to prevent render blocking
const criticalStyles = `
  .hero-fade-1 { animation: heroFadeUp 0.7s ease-out 0.5s both; }
  .hero-fade-2 { animation: heroFadeUp 0.7s ease-out 0.8s both; }
  .hero-fade-3 { animation: heroFadeUp 0.7s ease-out 1.1s both; }
  .hero-fade-4 { animation: heroFadeUp 0.7s ease-out 1.4s both; }
  .hero-fade-5 { animation: heroFadeUp 0.7s ease-out 1.7s both; }
`;

export async function HomeHeroFast({ locale }: { locale: string }) {
  ...
  return (
    <>
      ...
      <div className="relative flex h-[70vh] w-screen items-center justify-center">
        {/* Use native img for faster loading */}
        <div className="relative h-full w-full overflow-hidden">
          ...
          <img
            ...
            height="1080"
            srcSet={`
              ${heroImageUrl}?w=640&q=60 640w,
              ${heroImageUrl}?w=1024&q=60 1024w,
              ${heroImageUrl}?w=1920&q=60 1920w
            `}
